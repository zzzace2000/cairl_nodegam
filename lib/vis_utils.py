...
def vis_main_effects(all_dfs, num_cols=4, model_names=None, only_non_binary=False, call_backs=None, 
                     ...
                     only_interactions=False, sort_by_imp=False, show_imp_in_title=True):
    ...
    if top_interactions >= 0:
        if top_interactions == 0:
            ...
            if sort_by_imp:
                first_df = first_df.sort_values('importance', ascending=False)
        else:
    ...
    #                     tick.set_rotation(0)
            ...
            feat_names = [
                first_df[first_df.feat_idx == int(fid1)].feat_name.iloc[0],
                first_df[first_df.feat_idx == int(fid2)].feat_name.iloc[0],
            ]
        ...
        title = row.feat_name
        if 'importance' in row and show_imp_in_title:
