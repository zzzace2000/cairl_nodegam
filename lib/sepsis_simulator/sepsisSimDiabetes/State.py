...
class State(object):
    ...
    NUM_PROJ_OBS_STATES = int(NUM_OBS_STATES / 5)  # Marginalizing over glucose
    NUM_FULL_STATES = int(NUM_OBS_STATES * NUM_HID_STATES)
    PHI_DIM = 5 # the phi(s,a) dimension. See get_phi_vector()
    # Used for min-max normalization. The last dim is just uniform noise
    PHI_MAX = [2, 2, 1, 4, 1]
    PHI_MIN = [0, 0, 0, 0, 0]
    ...
    def __init__(self,
        ...
        if state_idx is not None:
            ...
            self.set_state_by_categs(state_categs, diabetic_idx)
    ...
    def set_state_by_categs(self, state_categs, diabetic_idx):
    ...
    def set_state_by_idx(self, state_idx, idx_type, diabetic_idx=None):
        ...
        """
        if idx_type == 'obs':
            term_base = self.NUM_OBS_STATES/3 # Starts with heart rate
        elif idx_type == 'proj_obs':
            term_base = self.NUM_PROJ_OBS_STATES/3
        elif idx_type == 'full':
            term_base = self.NUM_FULL_STATES/2 # Starts with diab
    ...
    def copy_state(self):
        return self.__class__(state_categs = [
            self.hr_state,
            self.sysbp_state,
            self.percoxyg_state,
            self.glucose_state,
    ...
    def get_state_vector(self):
        return np.array([self.hr_state,
            ...
            self.vaso_state,
            self.vent_state,
            self.diabetic_idx]).astype(int)

    def get_phi_vector(self):
        '''
        My own definition for phi that only includes states but
        not treatments. Used in IRL
        '''
        obs = self.get_state_vector()[:4]
        noise_obs = np.random.rand()
        obs = np.concatenate([obs, [noise_obs]])  # add noise dim
        return obs


class ConfoundState(State):
    '''
    We add a spurious correlation that the reward is correlated with such feature.
    - Need new transitions?
    - A reward causing which states it will get? A new transition model in disc state?
    '''
    NUM_OBS_STATES = State.NUM_OBS_STATES * 2
    NUM_HID_STATES = 2  # Binary value of diabetes
    NUM_PROJ_OBS_STATES = int(NUM_OBS_STATES / 5)  # Marginalizing over glucose
    NUM_FULL_STATES = int(NUM_OBS_STATES * NUM_HID_STATES)
    PHI_DIM = 5 # the phi(s,a) dimension. See get_phi_vector()
    # Used for min-max normalization. The last dim is the confounded state
    PHI_MAX = [2, 2, 1, 4, 1]
    PHI_MIN = [0, 0, 0, 0, 0]
    ...
    def set_state_by_idx(self, state_idx, idx_type, diabetic_idx=None):
        ...
        term_base = (self.NUM_FULL_STATES // 2)
        self.confound_state = state_idx // term_base
        state_idx = state_idx % term_base

        return super().set_state_by_idx(state_idx, idx_type, diabetic_idx=diabetic_idx)

    def set_state_by_categs(self, state_categs, diabetic_idx):
        assert len(state_categs) == 8, "must specify 8 state variables"

        self.confound_state = state_categs[-1]
        return super().set_state_by_categs(
            state_categs=state_categs[:-1],
    ...
    def get_phi_vector(self):
        ...
        # Add an additional
        obs = np.concatenate([obs, [self.confound_state]])
        return obs
