import numpy as np
from .State import State, ConfoundState
...
class MDP(object):
    ...
    def __init__(self, init_state_idx=None, init_state_idx_type='obs',
        ...
        num_states = self.state_cls.NUM_OBS_STATES
        if policy_idx_type == 'obs':
            num_states = self.state_cls.NUM_OBS_STATES
        elif policy_idx_type == 'full':
            num_states = self.state_cls.NUM_HID_STATES * self.state_cls.NUM_OBS_STATES
        elif policy_idx_type == 'proj_obs':
            num_states = self.state_cls.NUM_PROJ_OBS_STATES

        # Check the policy dimensions (states x actions)
        if policy_array is None:
            policy_array = np.ones((num_states, Action.NUM_ACTIONS_TOTAL)) \
                           / Action.NUM_ACTIONS_TOTAL
        else:
            assert policy_array.shape[0] == num_states
        ...
        * This option will set glucose to a normal level
        ...
        if state_idx is not None:
            if idx_type == 'obs' and diabetic_idx is not None:
                ...
                diabetic_idx = self.rng.binomial(1, self.p_diabetes)
            elif idx_type == 'full':
                option = 'spec_full'
        ...
        if option in ['random', 'random_cond_diab'] :
            ...
            init_state = self._get_new_state(
                state_idx=state_idx, idx_type=idx_type,
                diabetic_idx=diabetic_idx)

        return init_state

    def _get_new_state(self, state_idx=None, idx_type='obs', diabetic_idx=None, state_categs=None):
        return self.state_cls(
            state_idx=state_idx, idx_type=idx_type,
            diabetic_idx=diabetic_idx, state_categs=state_categs)
    ...
    def generate_random_state(self, diabetic_idx=None):
        ...
        hr_state = self.rng.choice(np.arange(3), p=np.array([.25, .5, .25]))
        sysbp_state = self.rng.choice(np.arange(3), p=np.array([.25, .5, .25]))
        # percoxyg w.p. [.2, .8]
        percoxyg_state = self.rng.choice(np.arange(2), p=np.array([.2, .8]))

        if diabetic_idx == 0:
            glucose_state = self.rng.choice(np.arange(5), \
        ...
        return self._get_new_state(state_categs=state_categs, diabetic_idx=diabetic_idx)
    ...
    def transition_antibiotics_on(self):
        ...
        if self.state.hr_state == 2 and self.rng.uniform(0,1) < 0.5:
            self.state.hr_state = 1
        if self.state.sysbp_state == 2 and self.rng.uniform(0,1) < 0.5:
    ...
    def transition_antibiotics_off(self):
        ...
        if self.state.antibiotic_state == 1:
            if self.state.hr_state == 1 and self.rng.uniform(0,1) < 0.1:
                self.state.hr_state = 2
            if self.state.sysbp_state == 1 and self.rng.uniform(0,1) < 0.1:
    ...
    def transition_vent_on(self):
        ...
        if self.state.percoxyg_state == 0 and self.rng.uniform(0,1) < 0.7:
    ...
    def transition_vent_off(self):
        ...
        if self.state.vent_state == 1:
            if self.state.percoxyg_state == 1 and self.rng.uniform(0,1) < 0.1:
    ...
    def transition_fluctuate(self, hr_fluctuate, sysbp_fluctuate, percoxyg_fluctuate, \
        ...
        if glucose_fluctuate:
            glucose_prob = self.rng.uniform(0,1)
            if self.state.diabetic_idx == 0:
                if glucose_prob < 0.1:
    ...
    def select_actions(self):
        ...
        aev_idx = self.rng.choice(np.arange(Action.NUM_ACTIONS_TOTAL), p=probs)
        return Action(action_idx = aev_idx)


class GAMMDP(MDP):
    '''
    Modify rewards that depend on each feature (like GAM)
    '''
    MAX_REWARD = 0.5
    MIN_REWARD = (0.5 - 1 - 1.2 - 1 - 0.8)
    ...
    def calculateReward(self):
        ...
        reward = 0.5
        reward += hr_state_dict[self.state.hr_state]
        reward += sysbp_state_dict[self.state.sysbp_state]
        reward += percoxyg_state[self.state.percoxyg_state]
        reward += glucose_state_dict[self.state.glucose_state]
        return reward


class LinearMDP(MDP):
    '''
    Modify rewards that linearly depend on each feature
    '''
    MAX_REWARD = (0.5 - 0.3 - 0.4 + 0.6 + 0.8)
    MIN_REWARD = (0.5 - 0.9 - 1.2 + 0 + 0)
